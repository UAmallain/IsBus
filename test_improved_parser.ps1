...
try {
    $response = Invoke-RestMethod -Uri "http://localhost:5000/api/parser/parse" -Method Post -Body '{"input":"test 506 123-4567"}' -ContentType "application/json" -ErrorAction Stop
...
$testCases = @(
    ...
    @{
        ...
        Description = "Unit number + civic number (223 1057)"
    },
    @{
...
Write-Host "  1. Prefers longest street name matches (Indian Mountain Road vs Mountain Road)"
Write-Host "  2. Handles unit + civic numbers (e.g., 223 1057 Beaverhill)"
Write-Host "  3. Better business name recognition (Atlantic, Pacific, Quality, etc.)"
Write-Host ""
